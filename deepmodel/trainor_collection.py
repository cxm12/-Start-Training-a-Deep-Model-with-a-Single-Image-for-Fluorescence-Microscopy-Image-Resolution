...
import deepmodel.loss_collection as lc
...
def Flop():
    run_meta = tf.RunMetadata()  # profiler.profile()
    opts = tf.profiler.ProfileOptionBuilder.float_operation()
    flops = tf.profiler.profile(graph=tf.keras.backend.get_session().graph, run_meta=run_meta,
                               cmd='op', options=opts)
    param = tf.profiler.profile(graph=tf.keras.backend.get_session().graph, run_meta=run_meta,
                               cmd='op', options=tf.profiler.ProfileOptionBuilder.float_operation())
    return flops.total_float_ops  # param.total_parameters=0
...
class core_trainer:
    ...
    def __init__(
        ...
        self.output_model_file_path = os.path.join(self.output_dir,
            self.model_string + "_model.h5"
            # self.run_uid + "_" + self.model_string + "_model.h5"
        )
        print('self.output_model_file_path', self.output_model_file_path)
        ...
        if self.nb_gpus > 1:
            mirrored_strategy = tensorflow.distribute.MirroredStrategy()  # TensorFlow分布式训练
        ...
        if 'model_path' in json_data.keys() and os.path.exists(json_data['model_path']):
            self.local_model = load_model(json_data['model_path'],
                custom_objects={"annealed_loss": lc.loss_selector("annealed_loss")},)
            print('****** Load Model', json_data['model_path'], '********')
        else:
            print('****** Not Load Model = train from start ********')
        # print('Model Size = ', size(self.local_model))
        # print('Flop = ', Flop())
        # exit()

    def compile(self):
        self.local_model.compile(loss=self.loss, optimizer=self.optimizer)  # , metrics=['mae'])
    ...
    def initialize_callbacks(self):
        checkpoint_path = os.path.join(
            ...
            # self.run_uid + "_" +
            self.model_string +
            "-ep{epoch:04d}-loss{val_loss:.4f}.h5",
        )
    ...
    def run(self):
        ...
        if self.steps_per_epoch > 0:
            # self.model_train = self.local_model.fit_generator(
