...
# [B, 7, 7, 7, in_ch] -> [B, 7, 7, 7, in_ch]
def fmri_unet_denoiser_F3(path_json):
    def local_network_function(input_img):
        try:
            d = input_img.shape[-1].value
        except:
            d = input_img.shape[-1]
        # encoder
        conv1 = Conv3D(8, (3, 3, 3), activation="relu", padding="same")(input_img)
        ...
        out = Conv3D(d, (1, 1, 1), activation=None, padding="same")(conv5)
        return out
...
# 512 x 512 x number_img_in  -> 512 x 512 x 1
...
# 512 x 512 x number_img_in  -> 512 x 512 x number_img_in
def unet_single_1024_F20(path_json):
    def local_network_function(input_img):
        try:
            d = input_img.shape[-1].value
        except:
        ...
        decoded = Conv2D(d, (1, 1), activation=None, padding="same")(
            conv10
        )  # 512 x 512 x 1
...
# [B, 7, 7, in_ch] -> [B, 7, 7, 1]
def deepcad_unet_denoiser(path_json):
    # based on unet_single_1024
    def local_network_function(input_img):
        # encoder
        # input = 160 x 512 x number_img_in (wide and thin)
        conv1 = Conv2D(64, (3, 3), activation="relu", padding="same")(
            input_img
        )
        # 80 x 512 x 32
        ...
        # 20 x 128 x 128 (small and thick)
        pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
        conv4 = Conv2D(512, (3, 3), activation="relu", padding="same")(
            pool3
        )
        # 10 x 128 x 128 (small and thick)
        pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
        conv5 = Conv2D(1024, (3, 3), activation="relu", padding="same")(
            pool4
        )  # 128 x 128 x 128 (small and thick)
