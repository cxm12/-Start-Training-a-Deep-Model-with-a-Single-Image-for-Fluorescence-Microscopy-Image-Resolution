...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    ...
    """
    assert ('c' in order) or ('t' in order), "Conv layer MUST be present"
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            if 'c' in order:
                ...
                is_before_conv = i < order.index('t')
            ...
            raise ValueError(f"Unsupported layer type '{char}'. MUST be one of ['b', 'g', 'r', 'l', 'e', 'c', 't']")
